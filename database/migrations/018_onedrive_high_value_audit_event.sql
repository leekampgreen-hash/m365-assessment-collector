...
CREATE TABLE IF NOT EXISTS core.onedrive_high_value_audit_event (
    ...
    workload TEXT NOT NULL CHECK (workload = 'OneDrive'),
    record_type TEXT NULL,
    actor_upn TEXT NULL,
    event_category TEXT NOT NULL CHECK (event_category IN ('EXTERNAL_SHARING', 'MALWARE_DETECTED')),
...
CREATE INDEX IF NOT EXISTS onedrive_high_value_audit_event_tenant_event_time_idx
    ON core.onedrive_high_value_audit_event (tenant_id, event_time);
CREATE INDEX IF NOT EXISTS onedrive_high_value_audit_event_tenant_category_event_time_idx
    ON core.onedrive_high_value_audit_event (tenant_id, event_category, event_time);

REVOKE ALL ON core.onedrive_high_value_audit_event FROM PUBLIC;
GRANT SELECT, INSERT ON core.onedrive_high_value_audit_event TO graph_agent_runtime;
DO $$
BEGIN
    EXECUTE format(
        'GRANT USAGE, SELECT ON SEQUENCE %s TO graph_agent_runtime',
        pg_get_serial_sequence('core.onedrive_high_value_audit_event', 'onedrive_high_value_audit_event_id')
    );
END;
$$;
