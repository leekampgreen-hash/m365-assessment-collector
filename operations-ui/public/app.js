...
async function loadNewPanels() {
  ...
  const p = pim.data || pim; const assignments = p.assignments || p.assignment_details || p.assignments_list || []; const assignmentType = (x) => String(x.assignment_type || x.type || "").toLowerCase(); const permanent = p.permanent ?? p.permanent_count ?? assignments.filter((x) => ["assigned", "permanent"].includes(assignmentType(x))).reduce((sum, x) => sum + Number(x.count ?? 1), 0); const eligible = p.eligible ?? p.eligible_count ?? assignments.filter((x) => ["eligible", "time-bound"].includes(assignmentType(x))).reduce((sum, x) => sum + Number(x.count ?? 1), 0); panelCards("#pim-cards", [["👑","Total assignments",p.total ?? 0],["🔒","Permanent assignments",permanent,"",permanent > 3?"unavailable":""],["⏱️","Eligible (time-bound)",eligible],["🎯","Unique roles",p.unique_roles ?? new Set(assignments.map((x) => x.role_display_name || x.role)).size]]); paginatedTable("#pim-table", ["Principal","Role","Type","Start Date","End Date"], assignments.map((x) => `<tr class="${["assigned", "permanent"].includes(assignmentType(x))?"warning":""}"><th>${escapeHtml(x.principal_display_name || "Hidden")}</th><td>${escapeHtml(x.role_display_name || x.role)}</td><td>${escapeHtml(x.assignment_type || x.type)}</td><td>${escapeHtml(x.start_date)}</td><td>${escapeHtml(x.end_date)}</td></tr>`), "pim");
