...
class ManagementActivityTransport:
    ...
    def collect(self, start: datetime, end: datetime, metrics: AuditMetrics | None = None) -> tuple[AuditContent, ...]:
        ...
        while url:
            ...
            if next_uri is not None and (not isinstance(next_uri, str) or not next_uri.startswith("https://manage.office.com/")): raise AuditTransportError("SCHEMA_CONTRACT_FAILURE", "malformed NextPageUri")
            url = next_uri
        return tuple(out)


def collect_and_persist_onedrive_audit(*, tenant_id: int, auth_config: CollectorAuthConfig, connection: Any, url_open: Callable[..., Any], start: datetime, end: datetime, collected_at: str, collection_run_id: int | None = None, endpoint_run_id: int | None = None,     overlap: timedelta = DEFAULT_OVERLAP, max_window: timedelta = DEFAULT_MAX_WINDOW, dry_run: bool = False, initial_lookback: timedelta = DEFAULT_INITIAL_LOOKBACK) -> Mapping[str, Any]:
    ...
    checkpoint = get_onedrive_audit_checkpoint(connection, tenant_id=tenant_id)
    target_end = _utc(end)
    ...
    for record in records:
        if record.get("Workload") == "OneDrive": metrics.onedrive_records += 1
        row = normalize_onedrive_audit_record(record, tenant_id, collected_at, collection_run_id=collection_run_id, endpoint_run_id=endpoint_run_id)
        if row is not None: normalized.append(row); metrics.high_value_candidates += 1
        else: metrics.records_dropped_out_of_scope += 1
    metrics.normalized_events = len(normalized)
    metrics.checkpoint_proposed = target_end.isoformat()
    if not dry_run:
        result = persist_onedrive_high_value_audit_batch(connection, normalized, trusted_tenant_id=tenant_id)
        metrics.inserted, metrics.duplicate_skips = result.inserted, result.duplicate_skips
        advance_onedrive_audit_checkpoint(connection, tenant_id=tenant_id, checkpoint_at=target_end)
...
def normalize_onedrive_audit_record(record: Mapping[str, Any], tenant_id: int, collected_at: str, *, collection_run_id: int | None = None, endpoint_run_id: int | None = None) -> Mapping[str, Any] | None:
    if record.get("Workload") != "OneDrive": return None
    if not record.get("Id") or not isinstance(record.get("CreationTime"), str):
        if record.get("Operation") in {"AnonymousLinkCreated", "SharingInvitationCreated", "SharingSet", "FileMalwareDetected"}: raise AuditTransportError("SCHEMA_CONTRACT_FAILURE", "locked candidate is malformed")
        return None
    ...
    elif operation == "FileMalwareDetected": category, external, anonymous = "MALWARE_DETECTED", False, False
    else: return None
    try: datetime.fromisoformat(record["CreationTime"].replace("Z", "+00:00"))
    except (TypeError, ValueError): return None
    return {"tenant_id": tenant_id, "audit_record_id": record["Id"], "event_time": record["CreationTime"], "operation": operation, "workload": "OneDrive", "record_type": record.get("RecordType"), "actor_upn": record.get("UserId"), "collection_run_id": collection_run_id, "endpoint_run_id": endpoint_run_id, "event_category": category, "external_flag": external, "anonymous_flag": anonymous, "collected_at": collected_at, "retention_class": "LONG", **{k: record.get(src) for k, src in {"client_ip":"ClientIP","object_id":"ObjectId","site_url":"SiteUrl","source_relative_url":"SourceRelativeUrl","source_file_name":"SourceFileName","unique_sharing_id":"UniqueSharingId","target_user_or_group_name":"TargetUserOrGroupName","target_user_or_group_type":"TargetUserOrGroupType"}.items()}}
